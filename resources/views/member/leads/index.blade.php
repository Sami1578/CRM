...
@section('content')
    @include('member.leads.modals.create')
    @include('member.leads.modals.changestatus')
    @include('member.leads.modals.send-message')
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).delegate('.changeStatus', 'click', function (e){
                ...
                let url = "{{ route('myleads.change.status', 'id') }}";
                url = url.replace('id', id);
            ...
            $('#change-status-form').submit(function (e){
                ...
                $.ajax({
                    data : formData,
                    url: "{{ route('myleads.update.status') }}",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).delegate('.message', 'click', function (e){
                ...
                let url = "{{ route('myleadmessages.edit', 'id') }}";
                url = url.replace('id', id);
                // console.log(url);
                $.get(url, function(data){
                    if(data.code != 100){
            ...
            $('#send-message-form').submit(function (e){
                ...
                $.ajax({
                    data : formData,
                    url: "{{ route('myleadmessages.store') }}",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    success:function (data){
                        ...
                        if(data.code != 100){
                            ...
                            $('.messages').empty();
                            let id = data.lead_id;
                            let reload_url = "{{ route('myleadmessages.edit', 'id') }}";
